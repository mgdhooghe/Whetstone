...
def thread_run(path, search, config, col_type, dataset, sampleset):
	...
	if train_method == "VTrain":
		...
		enc = VGAN_generator(x_dim, param["gen_hidden_dim"], param["z_dim"], param["gen_num_layers"], ["normalize"], [(0,param["z_dim"])])
		fd = VGAN_discriminator(x_dim, param["dis_hidden_dim"], param["dis_num_layers"])#,condition=True,c_dim=c_dim)
		C_Train(search, path, sample_it, gen, dis, enc, fd, config["n_epochs"], param["lr"], train_it, param["z_dim"], dataset, col_type, sample_times,itertimes = 100, steps_per_epoch = config["steps_per_epoch"],GPU=GPU,data=config["data"],protected=config["protected_var"])
