...
def organize_data(directory, file, ablation):
	...
	if dataset_name != 'Adult':
		...
		select_from['GAN_METHOD'] = 'VGAN 1HOT NORM'
	else:
		select_from = select_from["WGAN-gmm"]	
		select_from['GAN_METHOD'] = 'WGAN 1HOT GMM'
	'''
	select_from = select_from.T
	select_from['GAN_METHOD'] = select_from.index
	select_from['GAN_METHOD'] = [ 'VGAN 1HOT NORM' if 'VGAN-norm' in x else x for x in select_from['GAN_METHOD']]
	select_from['GAN_METHOD'] = [ 'VGAN 1HOT GMM' if 'VGAN-gmm' in x else x for x in select_from['GAN_METHOD']]
	select_from['GAN_METHOD'] = [ 'WGAN 1HOT NORM' if 'WGAN-norm' in x else x for x in  select_from['GAN_METHOD']]
	select_from['GAN_METHOD'] = [ 'WGAN 1HOT GMM' if 'WGAN-gmm' in x else x for x in select_from['GAN_METHOD']]
	select_from['GAN_METHOD'] = [ 'VGAN ORDI NORM' if 'VGAN-ordi-norm' in x else x for x in select_from['GAN_METHOD']]
	select_from['GAN_METHOD'] = [ 'VGAN ORDI GMM' if 'VGAN-ordi-gmm' in x else x for x in select_from['GAN_METHOD']]
	select_from.index = ["gan"]*len(select_from.index)
	...
	if 'FAIR_SWAP' in ablation or 'ALTERNATIVES' in ablation:
		...
		data.index = [ '\sys NAIVE' if '2' in txt else txt for txt in data.index ]
	...
	if 'select_percent' in ablation: 
		...
		data.index = [ 'base10' if txt == 'base' else txt for txt in data.index ]
	...
	data['GAN_METHOD'] = [ re.sub('\d+','',x) for x in data.index]
	data.index = [ re.search(r'\d+',x).group() if re.search(r'\d+',x) else x for x in data.index]
	data['GAN_METHOD'] = [ 'VGAN 1HOT NORM' if 'vgannorm' in x else x for x in data['GAN_METHOD'] ]
	data['GAN_METHOD'] = [ 'VGAN 1HOT GMM' if 'vgangmm' in x else x for x in data['GAN_METHOD'] ]
	data['GAN_METHOD'] = [ 'WGAN 1HOT NORM' if 'wgannorm' in x else x for x in data['GAN_METHOD'] ]
	data['GAN_METHOD'] = [ 'WGAN 1HOT GMM' if 'wgangmm' in x else x for x in data['GAN_METHOD'] ]
	data['GAN_METHOD'] = [ 'VGAN ORDI NORM' if 'vganordinorm' in x else x for x in data['GAN_METHOD'] ]
	data['GAN_METHOD'] = [ 'VGAN ORDI GMM' if 'vganordigmm' in x else x for x in data['GAN_METHOD'] ]
	if dataset_name == 'Adult':
		data['GAN_METHOD'] = [ 'WGAN 1HOT GMM' if 'base' in x else x for x in data['GAN_METHOD'] ]
	else:
		data['GAN_METHOD'] = [ 'VGAN 1HOT NORM' for x in data['GAN_METHOD'] ]
		data.loc['original']['GAN_METHOD'] = 'original'
...
def plot_data(data_folders, data_files, acc_measure, ablation):
	...
	for directory in data_folders:
		...
		all_data, dataset_name, og_name, gan_data, protected = organize_data(directory, file, ablation)
		for gan_method in all_data['GAN_METHOD'].unique():
			og_data = all_data[all_data.index == 'original']
			og_data['GAN_METHOD'] = gan_method
			fig, ax = plt.subplots(1 ,1, sharex=True, sharey=True)
			XLAB = ax.set_xlabel('Disparate Impact', fontweight="bold")
			i = 0
			...
			if acc_measure == 'bal_acc':
				ax_alias.set_ylabel('Balaced Accuracy', fontweight="bold")
			else:
				ax_alias.set_ylabel('Accuracy', fontweight="bold")
			
			data= all_data[all_data['GAN_METHOD'] == gan_method]
			#data= pd.concat([data,og_data])
			
			####### PLOT SAVED AS ########
			fig_name = 'mode_collapse/'+ablation+'_'+dataset_name+'_'+gan_method.replace(' ','-')+'_'+protected+'_'+acc_measure+'_plot.png'
			TITLE = ax_alias.set_title(dataset_name+': '+gan_method.replace('1HOT',''), fontweight="bold")
			...
			'''
			if 'beta' in ablation:
				keep = [ ind != 5 for ind in data.index ]
				data = data[keep]
			'''

			if "gan" not in data.index.array:
				data = pd.concat([gan_data[gan_data['GAN_METHOD'] == gan_method],data])	

			marker_size = 75

			data.index = [str(txt) for txt in data.index]
			c_i = 1
			ax_alias.scatter(og_data['DI'][0], og_data[acc_measure][0], color='k', s=marker_size)
			for uind in data.index: 
			...
			'''
			try:
				lgd = plt.legend(['Original']+list(data.index.array), loc='lower center', ncol=8, bbox_to_anchor=(0.5,-0.25), columnspacing=.8)
			...
			## SAVE PLOT
			#TITLE = plt.suptitle(dataset_name,fontweight="bold")
...
def paper_latx(latx_data, latx_label, f, write, last=False):
	...
	latx_data[['GAN','Cat.','Num.']] = latx_data['GAN'].str.split(' ',expand=True)
	latx_data = latx_data.set_index(['Dataset','GAN','Cat.','Num.'])
...
for metric in ['DI', 'bal_acc', 'acc', 'Stat Par', 'Eq Opp', 'Avg Odds', 'Theil']:
	## GET AVERAGE DATA
	di_data = data_dict['bal_acc'][['dataset',metric,metric+'_sem','GAN_METHOD']]
	...
	di_data = di_data[di_data['GAN_METHOD'] != 'original']
	#di_data = di_data.drop_duplicates()
	di_data = di_data.reset_index().pivot(index=['dataset','GAN_METHOD'], columns='index',values=metric)
	...
	di_data.index.names = ['Dataset','GAN']
